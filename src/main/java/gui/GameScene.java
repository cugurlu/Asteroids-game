...
public class GameScene extends JPanel {
    ...
    private void showGameScore(Graphics graphics) {
        if (!game.checkGameOver()) {
            ...
            graphics.drawString("Nickname: " + nickname, 10, 50);
        }
    }
    ...
    private void drawLives(Graphics2D graphics2D) {
        for (int i = 0; i < game.getPlayerLives(); i++) {
            ...
            graphics2D.drawImage(rocket, 2, 30, this);
    ...
    private void drawObjects(Graphics2D graphics2D, AffineTransform identity) {
        game.getExistingObjects().stream().filter(gameObject ->
                gameObject != game.getPlayerRocket()
                        || game.canDrawPlayer()).forEach(gameObject -> {
                            Vector gameObjectPosition = gameObject.getObjectPosition();
                            rotateObjects(graphics2D, gameObject,
                                    gameObjectPosition.axis1, gameObjectPosition.axis2);
